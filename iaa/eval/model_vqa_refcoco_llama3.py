...
class CustomDataset(Dataset):
    def __init__(self, questions, image_folder, is_all_img_resize_672, tokenizer, image_processor, model_config, args):
        self.questions = questions
        self.image_folder = image_folder
        self.tokenizer = tokenizer
        self.image_processor = image_processor
        self.model_config = model_config
        self.is_all_img_resize_672 = is_all_img_resize_672
        self.args = args
    ...
    def __getitem__(self, index):
        ...
        qs = 'Please provide the bounding box coordinate of the region this sentence describes: '+line["expression"]
...
# DataLoader
def create_data_loader(questions, image_folder, is_all_img_resize_672, tokenizer, image_processor, model_config,args, batch_size=1, num_workers=8):
    assert batch_size == 1, "batch_size must be 1"
    dataset = CustomDataset(questions, image_folder, is_all_img_resize_672, tokenizer, image_processor, model_config,args)
...
def eval_model(args):
    ...
    data_loader = create_data_loader(questions, args.image_folder, args.is_all_img_resize_672, tokenizer, image_processor, model.config,args)
    
    all_outputs = []
    for (input_ids, image_tensor), line in tqdm(zip(data_loader, questions), total=len(questions)):

        idx = line["sent_id"]
        cur_prompt = line["expression"]
